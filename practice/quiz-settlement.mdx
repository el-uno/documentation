---
title: "Quiz — Arc settlement: saga, rails, chains and compensation"
sidebarTitle: "Settlement Quiz"
description: "Twelve questions on the Arc settlement saga, payment rails, on-chain finality and compensation logic. Commit to an answer before opening each accordion."
icon: "circle-question"
---

Commit to an answer before opening each accordion. The questions cover the full settlement path — from compliance pre-check through on-chain finality to compensation and failure modes. Quiz · 12 questions · ~15 minutes.
...
<AccordionGroup>
  ...
  <Accordion title="11 · Idempotency keys are applied at which layers?" icon="circle-question">
    **All four: API, rail, chain, and event handlers.**

    Each layer can receive a retry independently. The API key deduplicates duplicate client requests. The rail key ensures the same payout instruction is not submitted twice even if the saga retries. The chain layer uses a nonce-based key so a resubmitted broadcast does not create a second transaction. Event handlers key on the event ID so a redelivered webhook does not double-post a journal.

    The plausible wrong answer is "just the API layer" — which protects against client retries but leaves the saga free to fan out duplicate work at every downstream boundary.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="The stories" icon="book" href="/stories">
