---
title: "Contexts, Events, and the Boundary That Fails Builds"
sidebarTitle: "Events"
description: "The transactional outbox, at-least-once delivery with effectively-once processing, and a boundary rule that becomes a build failure."
---

Six bounded contexts that never import each other's code have to communicate somehow. Arc uses two mechanisms, and which one applies is decided by a single question: **can the caller wait?** Events handle everything asynchronous; ports handle the cases where the caller needs an answer before proceeding. Understanding both mechanisms — and why the boundary rule that separates them requires two enforcement layers, not one — is the foundation for reading everything else in the architecture.
...
The worked example is account provisioning — the smallest complete demonstration of the boundary holding:
...
Publishing after commit means the state lands, then the process dies before publishing — the ledger never provisions the account, and nothing is logged as an error because nothing errored. Publishing before commit means the event fires, subscribers act on it, then the transaction rolls back — downstream state now reflects something that never happened.
...
<AccordionGroup>
  ...
  <Accordion title="ESLint no-restricted-imports — catches package-name imports" icon="triangle-exclamation">
    ...
    This was discovered by writing a deliberate violation of each kind and checking that each mechanism fired. One did not. [The full story →](/stories/the-boundary-that-wasnt)
  </Accordion>
</AccordionGroup>
