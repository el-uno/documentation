---
title: "The Settlement Saga: Five Steps, Five Compensations"
sidebarTitle: "Settlement Saga"
description: "Five steps, five compensations, and why compensating in the wrong order produces a balanced ledger with a completely false audit trail."
---

How money actually crosses the corridor: quote, screen, reserve, swap, settle on-chain, pay out — with a compensating action at every step. There is no two-phase commit spanning M-Pesa and Ethereum, and there never will be. The saga pattern is the answer: run steps forward, and if one fails, run compensations backward. Not atomicity — eventual consistency with an explicit, auditable unwind. The claim this page exists to justify:

<Note>
**For every failure at every step of the saga, the ledger ends balanced in every currency and the sender is made whole.** That is asserted by the chaos suite, and the suite is mutation-checked.
</Note>
...
A database transaction gives you atomicity across things that share a transaction manager. A corridor transfer does not: it touches a ledger, a blockchain, and a foreign bank. There is no shared transaction manager, no rollback protocol that spans all three.
...
<Note>
**Compliance is a hard pre-condition, not a side channel.** A transfer that is rejected or flagged for review never reaches `reserve`, so no money moves at all. The alternative — screen asynchronously and claw back — is common and much worse: it means the failure mode is "we paid a sanctioned party and are now trying to recall it", which is a regulatory event rather than an engineering one.
</Note>
...
<Tabs>
  ...
  <Tab title="2 · swap">
    ...
    Two currencies, each balancing independently. **The EUR position account now carries a standing credit — that *is* the open FX exposure**, visible rather than implicit.
  </Tab>
  <Tab title="3 · settle">
...
- **If the original balanced, the reversal balances.** Flipping directions preserves the equality, so a compensation can never itself unbalance the ledger.
- **The pair nets to zero.** Every touched account returns to precisely where it was.
...
<Note>
**Gas was really spent.** Reversing it would misstate the expense, so the network-fee journal is **not tracked** and therefore never compensated. It balances on its own — Dr expense, Cr chain float — so the trial balance stays zero regardless.
</Note>
...
<Steps>
  ...
  <Step title="Convert at mid, convert at quoted, record the difference as the FX spread fee">
    The spread could have been left implicit in the quoted rate — the customer would see the same number. Computing it explicitly makes it a fee line and a ledger entry, so corridor P&L can attribute revenue between the corridor fee and the FX margin. Burying it in the rate would make the margin invisible to reporting.
  </Step>
</Steps>

Quotes carry a **30-second TTL** and `assertUsable` throws on an expired one. Rates move; honouring a stale quote is an unhedged loss. A property test asserts the customer is **never** given more than mid-market, across the full amount range.
...
Rails are **idempotent on the caller's key**: resubmitting returns the original receipt rather than paying twice. This is the single most important property of a payout API and the easiest to get wrong.
